...
function PlaqueField({ placements, curveSamples, selectablesRef }) {
  ...
  if (runtime.current.length !== built.length) {
    runtime.current = built.map(() => ({ textured: false, failed: false }));
  }

  const fontReady = useRef(false);
  useEffect(() => {
    let active = true;
    ensurePlaqueFont().then(() => {
      if (active) {
  ...
  const generateFace = (placement, index) => {
    ...
    let canvas;
    try {
      canvas = renderPlaqueCanvas(placement.text);
    } catch (error) {
      state.failed = true;
      console.error('Failed to render plaque face', placement.id, error);
      return;
    }
